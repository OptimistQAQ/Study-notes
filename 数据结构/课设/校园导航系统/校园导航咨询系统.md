...
姓名 | 任务
-|-
秦嘉豪| 负责任务分配，关节点等相关代码编写，后期汇总。
韩林志|负责最短路径算法相关代码的编写。
秦效国|负责所有路径算法相关代码的编写。
张竞文|负责景点信息收集、景点查询、最佳路径等相关代码的编写。

## 【实现思路】

先将任务分配下去，由大家一同实现各个子功能，其中包括关节点、最短路径、最佳路径和所有路径；最后再由我进行汇总。
...
int main()
{
	...
	IMAGE img_ba;
	initgraph(800, 600);
	loadimage(&img_ba, _T("D:\\Program Files (x86)\\VS2017projects\\Navigation_consultation\\Navigation_consultation\\3.png"), 800, 600);
	putimage(0, 0, &img_ba);
	//system("pause");
	Sleep(2000);
	closegraph();
